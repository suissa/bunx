...
## 7. Example manifest and import-all TypeScript fixture
...
The companion [`examples/import-all.ts`](../examples/import-all.ts) imports the runtime dependencies and usable dev tools so you can validate both cache population and module resolution:

```sh
cd examples
BUNX_HOME=/tmp/bunx-resilient-example bun x run import-all.ts
BUNX_HOME=/tmp/bunx-resilient-example bun x cache info
```

The `@types/*` dev dependencies are intentionally validated through `package.json` scanning rather than runtime imports because they do not expose runtime modules.
